...
`sena.toml` 中的 `[extension]` 表用于配置模板、静态资源、注入内容以及外部插件——这是 Book 模式下自定义构建行为的主要入口。
...
两者都以数组形式配置，数组中的每一项是一条外部命令；命令会按数组顺序依次执行，前一个命令的输出会作为后一个命令的输入。这些命令与 mopress 之间通过标准输入/标准输出上的 JSON 协议通信，完整说明请参见 [外部插件协议](../custom/plugins/protocol.md)；更多插件形式（不局限于外部进程）请参见 [插件总览](../custom/plugins.md)。
