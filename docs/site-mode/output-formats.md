...
`unify` 这个 step（要求数据类型实现 `Thingable` 类型类）通常是管线的最后一步，负责把最终的数据（ `String`、 `Bytes`，或者 `Json`）转换为对应的 `Thing`。 `Thingable` 目前为这三种类型提供了默认实现，分别对应转换为 `Doc`、 `Asset`，以及 `Json` 自身的相应表示。
...
通过 `Multiple` 可声明多个产出文件，以下是将 `SUMMARY.md` 同时渲染为一份搜索索引 JSON 和一份 404 页面的示例：
...
一般地，MoPress 只允许有限地修改输出文件的扩展名而不允许修改文件名本身。若需修改可技巧性借助 `Multiple`，如：
...
Glob(
  ...
  Text(raw => {
    raw.map(text => {
      @core.Multiple([("bar.html", text |> @core.Doc)])
    })
  })
)
```
