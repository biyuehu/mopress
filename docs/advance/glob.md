...
Glob 匹配用于按路径规则匹配文件，在 MoPress 中出现在两处：Book 模式扩展配置中的静态资源字段，以及 Site 模式的 `Rule::Glob`、`Rule::Guard`。本页是这套语法的独立参考。
...
`glob_match` 判断给定路径是否匹配给定的 Glob 匹配，返回布尔值。`glob_match_with` 在此基础上追加一层自定义判定：`f` 会收到与 `glob_match` 相同的候选完整相对路径，其返回值决定该路径是否被视为匹配，这正是 `Rule::Guard` 底层依赖的匹配方式。`scan_files` 递归扫描 `root` 目录，返回其中所有匹配 `pattern` 的文件路径数组；`scan_files_with` 是它的自定义判定版本，`f` 同样收到每个候选文件相对于 `root` 的完整路径。两个扫描函数都可能因为目录无法读取（不存在、无权限等）而抛出 `@fs.IOError`。
...
| `src/?.md` | 匹配源目录下文件名恰好为单个字符加 `.md` 的文件，如 `src/a.md` |
